...
//
//        assertThat(oneCommentArticle.getId()).isEqualTo(threeCommentArticle.getId());
//        assertThat(threeCommentArticle.getId()).isNotNull();
//        assertThat(threeCommentArticle.getComments()).hasSize(3);
//        assertThat(((List)articleRepository.findAll()).size()).isEqualTo(2);
    }
...
//        assertThat(persistArticle).isNotNull();
    ...
    void deleteAuthorAfterArticleIsDeleted() {
        Author author = authorRepository.save(new Author("브라운"));
        Article article = articleRepository.save(new Article("title", "content", new AuthorRef(author.getId())));

//        Article article = articleRepository.save(new Article("title", "content", new Author("브라운")));
